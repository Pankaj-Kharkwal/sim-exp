...
// Helper to ensure outputs are in the correct format
const normalizeOutputs = (outputs: string | string[] | Record<string, BlockOutput>): Record<string, BlockOutput> => {
  if (typeof outputs === 'string') {
    if (outputs === 'any') return { output: { type: 'any' } }; // Handle 'any' case
    return { [outputs]: { type: 'any' } };
  }
  if (Array.isArray(outputs)) {
    return outputs.reduce((acc, out) => {
      acc[out] = { type: 'any' };
      return acc;
    }, {} as Record<string, BlockOutput>);
  }
  // It's already in the correct format or it's an object that can be cast.
  return outputs as Record<string, BlockOutput>;
};
