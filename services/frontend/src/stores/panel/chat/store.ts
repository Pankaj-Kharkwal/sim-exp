    ...
    persist(
      (set, get) => ({
        ...
        clearChat: (workflowId: string | null) => {
          set((state) => {
            ...
            if (workflowId) {
              ...
              delete newConversationIds[workflowId]
              return {
                ...newState,
        ...
        getConversationId: (workflowId) => {
          const state = get()
          return state.conversationIds[workflowId] ?? null
        },

        setConversationId: (workflowId, conversationId) => {
          set((state) => {
            const newConversationIds = { ...state.conversationIds }
            newConversationIds[workflowId] = conversationId
