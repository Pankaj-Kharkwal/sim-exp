...
function applyEnvironmentOverrides(config: CopilotConfig): void {
  const chatProvider = validateProviderId(import.meta.env.VITE_COPILOT_CHAT_PROVIDER)
  if (chatProvider) {
    config.chat.defaultProvider = chatProvider
  } else if (import.meta.env.VITE_COPILOT_CHAT_PROVIDER) {
    logger.warn(
      `Invalid COPILOT_CHAT_PROVIDER: ${import.meta.env.VITE_COPILOT_CHAT_PROVIDER}. Valid providers: ${VALID_PROVIDER_IDS.join(', ')}`
    )
  }

  if (import.meta.env.VITE_COPILOT_CHAT_MODEL) {
    config.chat.defaultModel = import.meta.env.VITE_COPILOT_CHAT_MODEL
  }

  const chatTemperature = parseFloatEnv(
    import.meta.env.VITE_COPILOT_CHAT_TEMPERATURE,
  ...
  const chatMaxTokens = parseIntEnv(import.meta.env.VITE_COPILOT_CHAT_MAX_TOKENS, 'COPILOT_CHAT_MAX_TOKENS')
  if (chatMaxTokens !== null) {
    config.chat.maxTokens = chatMaxTokens
  }

  const ragProvider = validateProviderId(import.meta.env.VITE_COPILOT_RAG_PROVIDER)
  if (ragProvider) {
    config.rag.defaultProvider = ragProvider
  } else if (import.meta.env.VITE_COPILOT_RAG_PROVIDER) {
    logger.warn(
      `Invalid COPILOT_RAG_PROVIDER: ${import.meta.env.VITE_COPILOT_RAG_PROVIDER}. Valid providers: ${VALID_PROVIDER_IDS.join(', ')}`
    )
  }

  if (import.meta.env.VITE_COPILOT_RAG_MODEL) {
    config.rag.defaultModel = import.meta.env.VITE_COPILOT_RAG_MODEL
  }

  const ragTemperature = parseFloatEnv(
    import.meta.env.VITE_COPILOT_RAG_TEMPERATURE,
  ...
  const ragMaxTokens = parseIntEnv(import.meta.env.VITE_COPILOT_RAG_MAX_TOKENS, 'COPILOT_RAG_MAX_TOKENS')
  if (ragMaxTokens !== null) {
    config.rag.maxTokens = ragMaxTokens
  }

  const ragMaxSources = parseIntEnv(import.meta.env.VITE_COPILOT_RAG_MAX_SOURCES, 'COPILOT_RAG_MAX_SOURCES')
  if (ragMaxSources !== null) {
    config.rag.maxSources = ragMaxSources
  }

  const ragSimilarityThreshold = parseFloatEnv(
    import.meta.env.VITE_COPILOT_RAG_SIMILARITY_THRESHOLD,
  ...
  const streamingEnabled = parseBooleanEnv(import.meta.env.VITE_COPILOT_STREAMING_ENABLED)
  if (streamingEnabled !== null) {
    config.general.streamingEnabled = streamingEnabled
  }

  const maxConversationHistory = parseIntEnv(
    import.meta.env.VITE_COPILOT_MAX_CONVERSATION_HISTORY,
  ...
  if (import.meta.env.VITE_COPILOT_TITLE_GENERATION_MODEL) {
    config.general.titleGenerationModel = import.meta.env.VITE_COPILOT_TITLE_GENERATION_MODEL
  }
}
